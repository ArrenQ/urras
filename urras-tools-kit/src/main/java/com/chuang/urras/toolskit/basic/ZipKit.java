...
public class ZipKit {
	...
	private static void validateFile(File srcFile, File zipFile) {
		if (!srcFile.exists()) {
			throw new SystemErrorException(Result.FAIL_CODE, StringKit.format("File [{}] not exist!", srcFile.getAbsolutePath()));
		}

		try {
			// 压缩文件不能位于被压缩的目录内
			if (srcFile.isDirectory() && zipFile.getParent().contains(srcFile.getCanonicalPath())) {
				throw new SystemErrorException(Result.FAIL_CODE, "[zipPath] must not be the child directory of [srcPath]!");
			}

...
public interface HCache {
     ...
     */
    Optional<Boolean> delete(byte[] key);

	/**
	 * 删除 缓存
	 * @param key key列表，每个key都 不 支持正则匹配
	 */
	Optional<Long> delete(byte[]... key);

    /**
     * 通过正则key删除 缓存
     * @param keyPattern 支持正则的key
     */
	Optional<Long> deletePattern(byte[] keyPattern);
	 ...
	 * 更新 value
	 */
	Optional<Boolean> updateValue(byte[] key, byte[] field, byte[] value);
	 ...
	 * 删除 value
	 */
    Optional<Long> deleteValue(byte[] key, byte[] filed);
    ...
    Optional<Long> deleteField(byte[] key, byte[]... field);
}

...
public class BeanKit {
    ...
    public static <V> V convertObj(Object entity, Supplier<V> objNew) {
        if(null == entity) {
            return objNew.get();
        }
        V vo = objNew.get();
    ...
    public static <V> List<V> convert(Collection list, Supplier<V> objNew) {
        List<V> vlist = new ArrayList<>();
        for(Object obj: list) {
            vlist.add(convertObj(obj, objNew));
        }
    ...
    public static <C extends Collection<V>, V> C convert(Collection list, Supplier<C> collectionNew, Supplier<V> objNew) {
        ...
        for(Object obj: list) {
            collection.add(convertObj(obj, objNew));
        }
    ...
    public static Optional<PropertyDescriptor> getPropertyDescriptor(Class<?> clazz, final String fieldName) {
        ...
        for (PropertyDescriptor propertyDescriptor : propertyDescriptors) {
            if (BasicKit.equals(fieldName, propertyDescriptor.getName())) {
    ...
    public static void setProperty(Object obj, String fieldName, Object value) {
        getPropertyDescriptor(obj.getClass(), fieldName).ifPresent(property -> {
            try {
                property.getWriteMethod().invoke(obj, Convert.parse(property.getPropertyType(), value));
            } catch (Exception e) {
                e.printStackTrace();
            }
        });
    ...
    public static <T> Optional<T> mapToBean(Map<?, ?> map, Class<T> beanClass) {
        return ClassKit.newInstance(beanClass).map(t -> fillBeanWithMap(map, t));
    }
    ...
    public static <T> Optional<T> mapToBeanIgnoreCase(Map<?, ?> map, Class<T> beanClass) {
        return ClassKit.newInstance(beanClass).map(t -> fillBeanWithMapIgnoreCase(map, t));
    }
    ...
    public static <T> Optional<T> requestParamToBean(javax.servlet.ServletRequest request, Class<T> beanClass) {
        return ClassKit.newInstance(beanClass).map(bean -> fillBeanWithRequestParam(request, bean));
    }
    ...
    public static <T> Optional<T> toBean(Class<T> beanClass, @Nullable Function<String, Object> valueProvider) {
        return ClassKit.newInstance(beanClass).map(t -> fillBean(t, valueProvider));
    }
    ...
    public static <T> T copyOne(Object source) {
        try {
            ...
            throw new SystemWarnException(Result.FAIL_CODE, "复制对象失败");
        }
    }
    /**
     * 复制Bean对象属性<br>
                ...
                if (sourcePd != null) {
                    ...
                    if (readMethod != null && ClassKit.isAssignable(writeMethod.getParameterTypes()[0], readMethod.getReturnType())) {
                        try {
                            ...
                            throw new SystemErrorException(Result.FAIL_CODE, MessageFormat.format("Copy property [{0}] to [{1}] error: {2}", sourcePd.getName(), targetPd.getName(), ex.getMessage()), ex);
                        }
                    }
                }
            }

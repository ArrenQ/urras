...
public class ServiceImpl<M extends BaseMapper<T>, T> implements IService<T> {
    ...
    public IPage<T> pageByRowQuery(RowQuery rowQuery) {
        ...
        //order by
        BasicKit.foreach(rowQuery.getSorts(), sort -> {
        ...
        // 将所有filter 转化为条件
        BasicKit.foreach(rowQuery.getFilters(), filter -> filter.handle(query, currentModelClass()));
    ...
    @Override
    public Optional<T> getOne(Wrapper<T> queryWrapper, boolean throwEx) {
        if (throwEx) {
    ...
    @Override
    public <V> Optional<V> getObj(Wrapper<T> queryWrapper, Function<? super Object, V> mapper) {
        return Optional.ofNullable(SqlHelper.getObject(log, listObjs(queryWrapper, mapper)));
    }
